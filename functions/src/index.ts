  ...
  .onPublish(async (message, context) => {
    ...
    if (stravaActivity && stravaActivity.aspect_type === 'create') {
      ...
      if (user) {
        ...
        const userEmailAlerts = user.emailAlerts ?? false;

        let distance = 0;
        let movingTime = 0;
        let equipmentId = null;
        let equipmentName: string = '';
        let hasValidToken = isTokenValid(Number(expiresIn));
          ...
          try {
            ...
            await axios.default
              ...
              .then(async (response) => {
                ...
                const activityObject: ActivityInfo = {
                  userId,
                  userName,
                  userEmail,
                  userEmailAlerts,
              ...
              .then((maintenanceDoc) => {
                if (maintenanceDoc.exists) {
                  ...
                  if (maintenance) {
                    ...
                    if (activityInfo?.userEmailAlerts && !maintenance.isValid) {
                      publishMessage(
                        JSON.stringify({
                          ...maintenance,
